...
def taller_etl_unl_wap():
    ...
    def publish_with_spark(audit_result) -> Dict[str, int]:
        ...
        validar Quality Gate y cargar al esquema prod.
        """
        ...
        logger.info("Iniciando fase PUBLISH: Transformación, Quality Gate y enmascaramiento con Spark")
        ...
        try:
            ...
            # =========================
            invalid_amount_count = df_audit.filter(col("amount") < 0).count()
            null_user_count = df_audit.filter(col("user_id").isNull()).count()
            null_transaction_count = df_audit.filter(col("transaction_id").isNull()).count()

            if null_user_count > 0 or null_transaction_count > 0:
                logger.error("Quality Gate FALLÓ")
                logger.error(f"Negativos: {invalid_amount_count}")
                logger.error(f"user_id nulos: {null_user_count}")
                logger.error(f"transaction_id nulos: {null_transaction_count}")

                raise AirflowFailException(
                    f"Quality Gate bloqueante: "
                    f"{invalid_amount_count} negativos, "
                    f"{null_user_count} user_id nulos, "
                    f"{null_transaction_count} transaction_id nulos"
                )

            logger.info("Quality Gate aprobado")
            ...
            try:
                logger.info("Aplicando enmascaramiento...")
            ...
            logger.info(f"Publicando {new_records} registros en prod.raw_transactions")
            ...
            logger.info("Fase PUBLISH completada:")
    ...
    # -------------------------------------------------------------------------
    # FASE 6: REGISTRO DE LINAJE
    # -------------------------------------------------------------------------
    @task(task_id='record_lineage')
    def record_lineage(audit_result, publish_result, dbt_result):
        logger.info("Registrando linaje del pipeline")
        return True
